 ...
 *
 */
public class WebDataRadioButton extends MarkupContainer implements IFieldComponent, IDisplayData, IProviderStylePropertyChanges, IScriptChoiceMethods,
	INullableAware, ISupportWebBounds, IRightClickListener, ISupplyFocusChildren<Component>
{
	...
	private final MyRadioButton selector;
	...
	public WebDataRadioButton(IApplication application, String id, String text)
	{
		...
		selector = new MyRadioButton("check_" + id); //$NON-NLS-1$
		...
		if (eventExecutor.hasChangeCmd() || eventExecutor.hasActionCmd())
		{
			...
			form.addDelayedAction(new ServoyForm.IDelayedAction()
			{
				public void execute()
				{
					...
					eventExecutor.fireChangeCommand(value, newVal, false, WebDataRadioButton.this);

					//if change cmd is not succeeded also don't call action cmd?
					if (isValueValid)
					{
						eventExecutor.fireActionCommand(false, WebDataRadioButton.this);
					}
				}

				public Component getComponent()
				{
					return WebDataRadioButton.this;
				}
			});
		}
		else
	...
	public void setRightClickCommand(String rightClickCmd, Object[] args)
	{
		...
		add(new ServoyAjaxEventBehavior("oncontextmenu") //$NON-NLS-1$
		{
			...
			{
				eventExecutor.onEvent(JSEvent.EventType.rightClick, target, WebDataRadioButton.this,
			...
			public boolean isEnabled(Component component)
			{
				...
				{
					Object oe = WebDataRadioButton.this.js_getClientProperty("ajax.enabled");
