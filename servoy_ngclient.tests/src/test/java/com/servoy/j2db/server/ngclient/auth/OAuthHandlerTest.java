...
public class OAuthHandlerTest
{
	...
	public void testRefreshOAuth_withCustomOAuthConfig_serviceCreated_refreshFails() throws Exception
	{
		...
		boolean checked = invokeRefreshOAuthTokenIfPossible(solutionWithOAuth, result, oldToken, createMockRequestForLoginPage(), createMockResponse());
		assertFalse(checked);
	}
	...
	public void testRefreshOAuth_solutionWithoutOAuthConfig_returnsFalse() throws Exception
	{
		...
		SvyID oldToken = new SvyID(tokenStr);
		LoginResult result = LoginResult.needsLogin();
		boolean checked = invokeRefreshOAuthTokenIfPossible(noOAuthSolution, result, oldToken, createMockRequestForLoginPage(), createMockResponse());
		assertFalse(checked);
	}
	...
	{
		OAuthHandler handler = new OAuthHandler(solution);
		java.lang.reflect.Method m = OAuthHandler.class.getDeclaredMethod("refreshOAuthTokenIfPossible",
			LoginResult.class, SvyID.class, HttpServletRequest.class, HttpServletResponse.class);
		m.setAccessible(true);
		return ((Boolean)m.invoke(handler, result, oldToken, request, response)).booleanValue();
	}
