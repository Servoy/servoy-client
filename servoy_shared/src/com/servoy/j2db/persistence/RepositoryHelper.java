...
//			else if (name.equals("onAction") && persist instanceof Field)	retval = true; //$NON-NLS-1$
	...
	public static boolean shouldShow(String name, Element element, Class< ? > persistClass)
	{
		...
		if (name.equals("locked") || name.equals("formIndex")) //$NON-NLS-1$ //$NON-NLS-2$
		{
			return false;
		}
	...
	public static String getDisplayName(String displayName, Class< ? > persistClass)
	{
		...
		if (Form.class.isAssignableFrom(persistClass) && displayName.equals("dataSource")) //$NON-NLS-1$
		{
			return "table"; //$NON-NLS-1$
		}
		if ((Relation.class.isAssignableFrom(persistClass) || RelationItem.class.isAssignableFrom(persistClass) || Solution.class.isAssignableFrom(persistClass)) &&
			displayName.endsWith("DataSource")) return displayName.substring(0, displayName.length() - "DataSource".length()) + "Table"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		return displayName;
	}
