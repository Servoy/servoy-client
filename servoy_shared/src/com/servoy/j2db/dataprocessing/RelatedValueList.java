...
public class RelatedValueList extends DBValueList implements IFoundSetEventListener
{
		...
		IRecordInternal parentState) throws ServoyException
	{
		...
		for (int i = 1; i < relations.length; i++)
		{
			...
			QueryJoin join = SQLGenerator.createJoin(application, relations[i], lastTable, new QueryTable(foreignTable.getSQLName(), foreignTable.getCatalog(),
				foreignTable.getSchema()), globalScopeProvider);
			select.addJoin(join);
