...
//		setLightWeightPopupEnabled(false);
		...
		if (UIManager.getLookAndFeel().getClass().getName().toUpperCase().indexOf("AQUA") < 0) //$NON-NLS-1$
		{
			...
			{
				// if mac os and non aqua laf, cannot select in combo using mouse because this property is set to false in J2DBClient
	...
	public void setValidationEnabled(boolean b)
	{
		...
		else
		{
			...
			if (!isEnabled() && accesibleStateHolder.isAccessible() && scriptable != null) scriptable.setEditableInFindMode();
		}
		if (isEditable())
		{
