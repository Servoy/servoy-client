 ...
 */
	...
	{
		application = app;
		// use client timezone poperty, on a smart client this is just always false but there the the default and the application timezone is the same
			...
			while (it.hasNext())
			{
				...
				for (String element : raw.toString().split("\\|\\|")) //$NON-NLS-1$
				{
					...
					try
					{
						...
						{
							// useClientTimezone is a server property so it should be read as an inverted thing..
							// so if this is true then the date in the client is exactly the date from the database
							// so no conversion should be done.
							timeZone = null;
						}
							...
							switch (c.getDataProviderType())
							{
								...
								case IColumnTypes.DATETIME :
									...
									else
									{
										...
										Object tst = Column.getAsRightType(c.getDataProviderType(), c.getFlags(), initialObj1, format, c.getLength(), timeZone,
											false);
									...
									if (data2 != null)
									{
										...
										{
											Object dt = Column.getAsRightType(c.getDataProviderType(), c.getFlags(), data2, format, c.getLength(), timeZone,
												false);
								...
								default :
									operator = ISQLCondition.LIKE_OPERATOR;
									value = Column.getAsRightType(c.getDataProviderType(), c.getFlags(), data, format, c.getLength() + 2, timeZone, false);//+2 for %...%
							}
