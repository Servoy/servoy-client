	...
	ILinkListener, IProviderStylePropertyChanges, ISupportWebBounds, IRightClickListener, IDesignModeListener
{
	...
	private class ImageDisplay extends WebBaseButton implements IResourceListener
	{
		private static final long serialVersionUID = 1L;
		...
		public ImageDisplay(IApplication application, String id)
		{
			...
			add(new SimpleAttributeModifier("id", id + "_1")); //$NON-NLS-1$ //$NON-NLS-2$
			add(new StyleAppendingModifier(new Model<String>()
			{
				...
				public String getObject()
				{
					...
					if (resource != null)
					{
						if (resource.getWidth() < WebDataImgMediaField.this.js_getWidth() && !WebDataImgMediaField.this.js_isReadOnly() &&
							WebDataImgMediaField.this.js_isEnabled())
						{
							return "top: 0px;left: 51px;position: absolute"; //$NON-NLS-1$
						}
					}
