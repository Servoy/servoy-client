...
public abstract class FormManager implements PropertyChangeListener, IFormManager
{
	...
	/**
	 * @see com.servoy.j2db.IPrepareForSave#prepareForSave() public boolean prepareForSave() { FormController fc = ((FormController)getCurrentForm()); return fc
	 *      == null?true:fc.prepareForSave(); }
	 */

	/**
	 * @see com.servoy.j2db.IPrepareForSave#recordEditStart(com.servoy.j2db.dataprocessing.IRecord) public boolean recordEditStart(IRecord record) {
	 *      FormController fc = (FormController)getCurrentForm(); return fc == null?true:fc.recordEditStart(record); }
	 */
	...
	public boolean closeFormInDialogOrWindow(String dialogName, boolean closeAll)
	{
		IMainContainer container = null;

		if ((dialogName == null || dialogName.replace(" ", "").length() == 0) && closeAll == true) //$NON-NLS-1$ //$NON-NLS-2$
		{
			boolean result = true;

			Iterator<String> it = getOrderedContainers();
			boolean ret = true;
			while (it.hasNext())
			{
				...
				if (key != null)
				{
					...
					{
						ret = closeFormInDialog(mContainer);
						if (ret)
						{
		...
		else if (dialogName == null || dialogName.replace(" ", "").length() == 0) //$NON-NLS-1$ //$NON-NLS-2$
		{
			container = getMainContainer(currentContainer.getContainerName());
		}
		else
		{
			container = getMainContainer(dialogName);
		}

		if (container != null)
		{
			//don't do anything if it is the main container
			if (container == getMainContainer(null)) return false;

			if (!closeAll)
			{
				// legacy V3 behavior, when multiple forms are shown in same form, go back 1 in history
				History hist = ((FormManager)application.getFormManager()).getHistory(container);
				if (hist.getLength() > 1 && hist.getIndex() > 0)
				{
					storeDialogSize(container);
			...
			boolean ret = closeFormInDialog(container);
			if (ret)
			{
				// If dialog is closed then we must clear the complete history
				History hist = ((FormManager)application.getFormManager()).getHistory(container);
	...
	{
		return containers.keySet().iterator();
	}

	public abstract void storeDialogSize(IMainContainer container);

	public abstract boolean restoreWindowBounds(IMainContainer container);

	public abstract IMainContainer getOrCreateMainContainer(String name);

	public abstract boolean closeFormInDialog(IMainContainer container);

	public abstract void showFormInDialog(final String formName, Rectangle rect, String title, boolean resizeable, boolean showTextToolbar, boolean closeAll,
		boolean modal, String dialogName);

	public abstract void showFormInFrame(String formName, Rectangle bounds, String windowTitle, boolean resizeble, boolean showTextToolbar, String windowName);
	...
	protected boolean getDialogModalState()
	{
		return true;
	}
	...
	{
		return showFormInMainPanel(formName, getCurrentContainer(), null, true, application.getCurrentWindowName());
	}

	public void clearLoginForm()
	{
		makeSolutionSettings(application.getSolution());
	}
					...
					invokeLaterRunnables.add(new Runnable()
					{
						public void run()
						{
							...
							if (currentController != null && fp.getName().equals(currentController.getName()))
							{
								...
								application.setCurrentWindowName(dialogName);
							}
						}
					});
				}
	...
	public static class History
	{
		...
		public boolean go(int i)
		{
			...
			FormController fc = ((FormManager)application.getFormManager()).showFormInMainPanel(f, container, null, true, application.getCurrentWindowName());
