...
public class PageContributor extends WebMarkupContainer implements IPageContributorInternal
{
	...
	public PageContributor(final IApplication application, String id)
	{
		...
		add(new AbstractServoyDefaultAjaxBehavior()
		{
			...
			protected void respond(AjaxRequestTarget target)
			{
				if (localCopy != null)
				{
					((FormManager)application.getFormManager()).showFormInDialog(localCopy.formName, localCopy.r, localCopy.title, localCopy.resizeble,
						localCopy.showTextToolbar, localCopy.closeAll, localCopy.modal, localCopy.dialogName);
					localCopy = null;
					WebEventExecutor.generateResponse(target, findPage());
				}
			}
	...
	public void renderHead(HtmlHeaderContainer container)
	{
		...
		// This disables the notification from the browser when a page is unloaded while a modal window is being displayed.
		boolean useAJAX = Utils.getAsBoolean(application.getRuntimeProperties().get("useAJAX")); //$NON-NLS-1$
		if (useAJAX)
		{
			response.renderOnLoadJavascript("Wicket.Window.unloadConfirmation = false;"); //$NON-NLS-1$
		}
	...
	public void showFormInDialogDelayed(String formName, Rectangle r, String title2, boolean resizeble, boolean showTextToolbar, boolean closeAll,
		boolean modal, String dialogName)
	{
		delayedDialog = new DelayedDialog(formName, r, title2, resizeble, showTextToolbar, closeAll, modal, dialogName);
		getStylePropertyChanges().setChanged();
	}
	...
	public class DelayedDialog implements Serializable
	{
		...
		public DelayedDialog(String formName, Rectangle r, String title, boolean resizeble, boolean showTextToolbar, boolean closeAll, boolean modal,
			String dialogName)
		{
		...
		{
			return new DelayedDialog(formName, r, title, resizeble, showTextToolbar, closeAll, modal, dialogName);
		}
	}
}
