	...
	class DataRendererRecordModel extends RecordItemModel
	{
		...
		protected IRecordInternal getRecord()
		{
			...
			else
			{
				...
				Debug.error("No foundset in form found!"); //$NON-NLS-1$
				return null;
			}
		}
	}
...
//	}
	...
	public void print(boolean showDialogs, boolean printCurrentRecordOnly, boolean showPrinterSelectDialog, PrinterJob printerJob)
	{
		...
		if (page != null)
		{
			...
			page.setShowURLCMD(url, "_self", null, 0); //$NON-NLS-1$
		}
		else
		{
			try
	...
	public FormAnchorInfo getFormAnchorInfo(final boolean onlyChanged)
	{
		...
		while (e1.hasNext())
		{
			...
			{
				elements.put(ComponentFactory.getWebID(obj), (ISupportAnchors)obj);
			}
		}
			...
			public Object component(Component component)
			{
				...
				formPart.visitChildren(ISupportWebBounds.class, new IVisitor()
				{
					public Object component(Component comp)
					{
						...
						if (onlyChanged && !(comp instanceof WebTabPanel))
						{
						...
						if (obj != null)
						{
							...
							if (((anchors > 0 && anchors != IAnchorConstants.DEFAULT)) || (comp instanceof WebTabPanel))
							{
								Rectangle r = ((ISupportWebBounds)comp).getWebBounds();
								if (r != null)
								{
									if (anchors == 0) anchors = IAnchorConstants.DEFAULT;
									part.addAchoredElement(id, anchors, r, comp.getClass());
								}
	...
	public int getAnchors(String componentId)
	{
		...
		{
			IPersist obj = e1.next();
			if ((obj instanceof ISupportAnchors) && ComponentFactory.getWebID(obj).equals(componentId)) return ((ISupportAnchors)obj).getAnchors();
		}
