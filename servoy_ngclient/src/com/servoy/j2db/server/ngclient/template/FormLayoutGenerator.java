...
public class FormLayoutGenerator
{
	...
	{
		PersistIdentifier persistIdentifierForDesign = getDesignId(fe);
		String persistIdentifierForDesignStringified = (persistIdentifierForDesign != null ? persistIdentifierForDesign.toHTMLEscapedJSONString() : null);
		String name = fe.getName();
		if (persistIdentifierForDesignStringified != null)
		{

			writer.print("<div ng-style=\"layout('");
			writer.print(persistIdentifierForDesignStringified);
			writer.print("')\"");

			JSONObject ngClass = new JSONObject();
			ngClass.put("invisible_element",
				"<getDesignFormControllerScope().model('" + persistIdentifierForDesignStringified + "').svyVisible == false<".toString());
			ngClass.put("highlight_element",
				"<getDesignFormControllerScope().highlightElement('" + persistIdentifierForDesignStringified + "') == true<".toString());//added <> tokens so that we can remove quotes around the values so that angular will evaluate at runtime
		...
		if (persistIdentifierForDesignStringified != null)
		{
			...
			if (!isSelectable(fe, persistIdentifierForDesign))
			{
	...
	private static boolean isSelectable(FormElement fe, PersistIdentifier persistIdentifierForDesign)
	{
		boolean selectable = !fe.isFormComponentChild();
		if (!selectable)
		{
			if (persistIdentifierForDesign == null) selectable = true; // should never happen - unless we are not in form designer; then non selectable attr is not used anyway; so don't generate it
			else
			{
				boolean containsAutoGeneratedName = false;
				// so it is a form component child - there we filter out elements with default
				// name (for which the user did not assign a name, so their auto-generated name starts with svy_)
				String[] compAndPath = persistIdentifierForDesign.persistUUIDAndFCPropAndComponentPath();
				for (int i = compAndPath.length - 1; i >= 0 && !containsAutoGeneratedName; i--)
					containsAutoGeneratedName = compAndPath[i].startsWith(FormElement.SVY_NAME_PREFIX);

				selectable = !containsAutoGeneratedName;
			}
		}
		return selectable;
	}
...
//	}
	...
	public static void generateFormElement(PrintWriter writer, FormElement fe, Form form)
	{
		...
		PersistIdentifier persistIdentifierForDesign = getDesignId(fe);
		String persistIdentifierForDesignStringified = (persistIdentifierForDesign != null ? persistIdentifierForDesign.toHTMLEscapedJSONString() : null);
		if (persistIdentifierForDesign != null)
		{
			if (form.isResponsiveLayout())
			{
				writer.print(" svy-id='");
	...
	public static PersistIdentifier getDesignId(FormElement fe)
	{
