                            ...
                            if (opType === CHANGED) {
                                ...
                                for (i = startIndex; i <= endIndex; i++) {
                                    ...
                                    if (internalState.conversionInfo[i]) {
                                        currentClientValue[i] = $sabloConverters.convertFromServerToClient(changedData[relIdx], internalState.conversionInfo[i],
                                                                                    currentClientValue[i], componentScope, propertyContext);
                                ...
                                // apply conversions
                                if (granularOpConversionInfo) changedData = $sabloConverters.convertFromServerToClient(changedData, granularOpConversionInfo, undefined, componentScope, propertyContext);
                                ...
                                for (i = numberOfInsertedRows - 1; i >= 0 ; i--) {
                                    ...
                                    else if (granularOpConversionInfo && granularOpConversionInfo[i]) internalState.conversionInfo[startIndex] = granularOpConversionInfo[i];
