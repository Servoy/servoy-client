...
public class SortableCellViewHeaders extends WebMarkupContainer implements IProviderStylePropertyChanges
{
	 ...
	 */
	public SortableCellViewHeaders(WebCellBasedView view, String id, final PageableListView listView, AbstractBase cellview, IApplication application,
		String initiallySortedColumnId, boolean initialSortAsc, IHeaders headerManager)
	{
		...
		group = new SortableCellViewHeaderGroup(listView, cellview);
		if (initiallySortedColumnId != null) group.add(initiallySortedColumnId, initialSortAsc);
	}
	...
	private boolean resolve(MarkupStream markupStream, ComponentTag tag, String id)
	{
		if (tag.getName().equalsIgnoreCase("th")) //$NON-NLS-1$
		{
			...
			if ((th_component_id != null) && (get(th_component_id) == null))
			{
				...
				SortableCellViewHeader headerComponent = new SortableCellViewHeader(view, th_component_id, group, cellview, useAJAX, application);
		...
		try
		{
			...
			while (allElements.hasNext())
			{
				...
				if ((someElement instanceof Field || someElement instanceof GraphicalComponent || someElement instanceof Bean) &&
					id.equals(ComponentFactory.getWebID(someElement)) && (someElement instanceof ISupportName))
				{
