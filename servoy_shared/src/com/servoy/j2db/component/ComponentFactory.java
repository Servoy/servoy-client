...
{
	public static final String WEB_ID_PREFIX = "sv_"; //$NON-NLS-1$
	 ...
	 */
	public static final String GROUPID_COMPONENT_PROPERTY = "groupId"; //$NON-NLS-1$
	...
	public static String getWebID(IPersist meta)
	{
	...
	public static Component createDesignComponent(IApplication application, FlattenedSolution flattenedSolution, IPersist meta, Form form)
	{
		...
		if (meta.getTypeID() == IRepository.BEANS)
		{
			...
			IComponent comp = createBean(application, (Bean)meta, flattenedSolution);
			...
			switch (meta.getTypeID())
			{
				case IRepository.TABPANELS :
					...
					else
					{
						...
						tabs.addTab("position example", new JLabel("form will appear here", SwingConstants.CENTER)); //$NON-NLS-1$ //$NON-NLS-2$
...
//	}
		...
		boolean printing)
	{
		...
		switch (meta.getTypeID())
		{
			...
			case IRepository.BEANS :
				comp = createBean(application, (Bean)meta, null);
				break;
		...
		{
			((JComponent)comp).putClientProperty("Id", ComponentFactory.getWebID(meta));
		}
		return comp;
	}
	...
	public static Pair<FixedStyleSheet, javax.swing.text.Style> getStyleForBasicComponent(IServiceProvider sp, BaseComponent bc, Form form)
	{
		...
		if ((pair == null || pair.getRight() == null || (pair.getRight()).getAttributeCount() == 0))
		{
			...
			{
				pair = getStyleForBasicComponent(sp, bc, formHierarchy.get(i));
				if (pair != null && pair.getRight() != null && (pair.getRight()).getAttributeCount() != 0)
				{
					break;
				}
	...
	{
		String lookupName = "root"; //$NON-NLS-1$
...
//		}
		...
		try
		{
			...
			if (obj instanceof IServoyBeanFactory)
			{
				obj = ((IServoyBeanFactory)obj).getBeanInstance(application.getApplicationType(), (IClientPluginAccess)application.getPluginAccess(),
					new Object[] { ComponentFactory.getWebID(bean) });
			}
			...
			if (obj == null)
			{
				c = application.getItemFactory().createLabel(ComponentFactory.getWebID(bean), "bean missing " + bean.getBeanClassName());
			}
			else if (!(obj instanceof java.awt.Component) && !(obj instanceof IComponent))
			{
				c = application.getItemFactory().createInvisibleBean(ComponentFactory.getWebID(bean), obj);
			}
			else if (!(obj instanceof IComponent))
			{
				c = application.getItemFactory().createBeanHolder(ComponentFactory.getWebID(bean), (Component)obj);
			}
			else
			{
				c = (IComponent)obj;
	...
	{
		IRect panel = application.getItemFactory().createRect(getWebID(rec), rec.getShapeType());
...
//		JLabel panel = new JLabel("xxx");
	...
	{
		IComponent panel = application.getItemFactory().createShape(getWebID(rec), rec);
				...
				if (application instanceof IApplication && ((IApplication)application).isInDeveloper())
				{
					...
					else if (type != Types.OTHER && type != currentType)
					{
						...
						StringBuffer message = new StringBuffer(
							"The valuelist was already created for type: " + Column.getDisplayTypeString(((CustomValueList)list).getType())); //$NON-NLS-1$
...
//						}
		...
		boolean printing)
	{
		...
		{
			case Field.PASSWORD :
				fl = application.getItemFactory().createDataPassword(getWebID(field));
				break;
			case Field.RTF_AREA :
				fl = application.getItemFactory().createDataTextEditor(getWebID(field), RTF_AREA, field.getEditable());
			...
			case Field.HTML_AREA :
				fl = application.getItemFactory().createDataTextEditor(getWebID(field), HTML_AREA, field.getEditable());
				if (fl instanceof IScrollPane)
				{
					applyScrollBarsProperty((IScrollPane)fl, field);
				}
			...
			case Field.TEXT_AREA :
				fl = application.getItemFactory().createDataTextArea(getWebID(field));
				if (fl instanceof IScrollPane)
				{
					applyScrollBarsProperty((IScrollPane)fl, field);
				}
			...
			case Field.TYPE_AHEAD :
				...
				{
					fl = createTypeAheadWithValueList(application, field, dataProviderLookup, type, format);
					break;
				}
				else if (dp != null && dp.getColumnWrapper() != null && dp.getColumnWrapper().getRelations() == null)//only allow plain columns
				{
					fl = application.getItemFactory().createDataLookupField(getWebID(field), form.getServerName(), form.getTableName(),
			...
			default ://Field.TEXT_FIELD
				...
				{
					fl = application.getItemFactory().createDataField(getWebID(field));
				}
				break;
		}
...
//		fl.setOpaque(!field.getTransparent());
		...
		if (el != null) // el is an ActionListener
		{
			...
			Object[] cmds = combineMethodsToCommands(form, form.getOnElementFocusGainedMethodID(), "onElementFocusGainedMethodID", field, //$NON-NLS-1$
				field.getOnFocusGainedMethodID(), "onFocusGainedMethodID"); //$NON-NLS-1$
	 ...
	 * @return
	 */
	private static IFieldComponent createTypeAheadWithValueList(IApplication application, Field field, IDataProviderLookup dataProviderLookup, int type,
		String format)
	{
		IFieldComponent fl = null;
		...
		else if (valuelist.getValueListType() == ValueList.DATABASE_VALUES)
		{
			...
			try
			{
				...
				fl = application.getItemFactory().createDataLookupField(getWebID(field), lookupValueList);
			}
			catch (Exception e1)
			{
				Debug.error(e1);
		...
		{
			fl = application.getItemFactory().createDataLookupField(getWebID(field),
		...
		IDataProviderLookup dataProviderLookup)
	{
		...
		if (styleInfo != null)
		{
			...
			if (ss != null && s != null)
			{
				...
				//if (application.getApplicationType() != IApplication.WEB_CLIENT)
				{
					...
					if (mediaUrl != null && mediaUrl.toString() != null)
					{
						...
						if (start != -1)
						{
							...
							if (name.endsWith("')") || name.endsWith("\")")) name = name.substring(0, name.length() - 2); //$NON-NLS-1$
				...
				String transform = (String)s.getAttribute(CSS.getAttribute("text-transform")); //$NON-NLS-1$
				if (transform != null)
				{
		...
		if (label.getOnActionMethodID() != 0 && label.getShowClick())
		{
			...
			{
				l = application.getItemFactory().createScriptButton(getWebID(label));
			}
			else
			{
				l = application.getItemFactory().createDataButton(getWebID(label));
				IDataProvider dp = null;
				try
				{
		...
		else
		{
			if (label.getDataProviderID() == null && !label.getDisplaysTags())
			{
				l = application.getItemFactory().createScriptLabel(getWebID(label), (label.getOnActionMethodID() > 0));
			}
			else
			{
				l = application.getItemFactory().createDataLabel(getWebID(label), (label.getOnActionMethodID() > 0));
				IDataProvider dp = null;
				try
				{
					dp = dataProviderLookup == null ? null : dataProviderLookup.getDataProvider(label.getDataProviderID());
...
//		l.setOpaque(!label.getTransparent());
	...
	{
		ISplitPane splitPane = application.getItemFactory().createSplitPane(getWebID(meta), meta.getTabOrientation());
	...
	private static IComponent createTabPanel(IApplication application, Form form, TabPanel meta, IScriptExecuter el)
	{
		...
		{
			oldColor = UIManager.getColor("TabbedPane.selected"); //$NON-NLS-1$
			UIManager.put("TabbedPane.selected", meta.getSelectedTabColor());
		}
		int orient = meta.getTabOrientation();
		ITabPanel tabs = application.getItemFactory().createTabPanel(getWebID(meta), orient, meta.hasOneTab());
...
//							((IGraphicalComponent)l).setIcon(c);
	...
	private static IComponent createTab(IApplication application, Tab meta)
	{
		...
		if (f != null) toolTip += "Form: " + f.getName(); //$NON-NLS-1$
