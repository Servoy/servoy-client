...
public class SortableCellViewHeader extends WebMarkupContainer implements IProviderStylePropertyChanges, ISupportWebTabSeq
{
	...
	private final class LabelResolverLink extends ServoySubmitLink implements IComponentResolver
	{
		...
		{
			if (tag.getName().equalsIgnoreCase("div") && get("headertext") == null)
			{
				try
				{
					...
					while (it2.hasNext())
					{
						...
						if (componentId.equals(ComponentFactory.getWebID(element)))
						{
				...
				{
					Debug.error("can't create html table for " + cellview, ex);
				}
			}
			return false;
		}
	 ...
	 * @param cellview
	 */
	public SortableCellViewHeader(final WebCellBasedView view, final String id, final SortableCellViewHeaderGroup group, AbstractBase cellview,
		boolean useAJAX, IApplication application)
	{
		...
		headerColumnTable.add(labelResolver = new LabelResolverLink("sortLink", useAJAX, group, id));
		labelResolver.add(new AttributeModifier("class", true, group));
		...
		while (iter.hasNext())
		{
			...
			{
				if (id.equals(ComponentFactory.getWebID(element)))
				{
		...
		if (cellview instanceof Portal && !((Portal)cellview).getSortable())
		{
			...
			resizeBar = new WebMarkupContainer("resizeBar");
		}
		else
		{
		...
		if (!blockResize && (!(cellview instanceof Portal) || ((Portal)cellview).getResizeble()) && useAJAX)
		{
			...
			resizeBar.add(new AttributeModifier("style", true, new Model<String>("cursor: col-resize;")));
		}
		else
		{
			resizeBar.add(new AttributeModifier("style", true, new Model<String>("cursor: pointer;")));
		}
						...
						if (media != null)
						{
							add(new StyleAppendingModifier(new Model<String>()
							{
								...
								public String getObject()
								{
									...
									return "background-image: url(" + urlFor(iconReference) + "?id=" + media_id + //$NON-NLS-1$ 
	...
	public void setWidth(int width)
	{
		...
		final int clientWidth = width - 2 * TemplateGenerator.SORTABLE_HEADER_PADDING - borderWidth;
