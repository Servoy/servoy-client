...
public class JSApplication implements IReturnedTypesProvider
{
	...
	private FormAndComponent getTriggerNames()
	{
		...
		Debug.error("Shouldn't happen that the trigger element names are empty, is this called in a script that is triggered by solution startup?"); //$NON-NLS-1$
		...
		try
		{
			...
			if (vl != null)
			{
				...
				if (valuelist != null)
				{
					...
					retval.setPrototype(ScriptableObject.getClassPrototype(application.getScriptEngine().getSolutionScope(), "Array")); //$NON-NLS-1$
					for (int i = 0; i < valuelist.getSize(); i++)
					{
	...
	{
		boolean canResize = Utils.getAsBoolean(application.getSettings().getProperty("window.resize.location.enabled", "true")); //$NON-NLS-1$ //$NON-NLS-2$
		if (!canResize) return;
	...
	public int js_getWindowX(Object[] args)
	{
		...
		return application.getWindowBounds(dialogName).x;
	...
	public int js_getWindowY(Object[] args)
	{
		...
		return application.getWindowBounds(dialogName).y;
	}
	...
	public int js_getWindowWidth(Object[] args)
	{
		...
		return application.getWindowBounds(dialogName).width;
	}
	...
	public int js_getWindowHeight(Object[] args)
	{
		...
		return application.getWindowBounds(dialogName).height;
	}
	...
	public void js_setWindowSize(Object[] args)
	{
		...
		if (application instanceof ISmartClientApplication)
		{
			...
			((ISmartClientApplication)application).setWindowSize(width, height, replaceFailingCharacters(windowName));
		}
	...
	public void js_setWindowLocation(Object[] args)
	{
		...
		if (application instanceof ISmartClientApplication)
		{
			...
			((ISmartClientApplication)application).setWindowLocation(x, y, replaceFailingCharacters(windowName));
		}
	}
	...
	public void js_closeSolution(final Object[] solution_to_open_args)
	{
		...
		//application.closeSolution(false, solution_to_open_args);
		throw new ExitScriptException("Solution closed"); //$NON-NLS-1$
	}
	...
	{
		if (!application.isInDeveloper() || application.getSettings().getProperty("window.resize.location.enabled", "true").equals("true")) //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
		{
			application.logout(null);
		}
		else
		{
			application.reportInfo("Solution tried to logout"); //$NON-NLS-1$
		}
	}
	...
	public void js_exit()
	{
		if (!application.isInDeveloper() || application.getSettings().getProperty("window.resize.location.enabled", "true").equals("true")) //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
		{
			...
			throw new ExitScriptException("Application exit"); //$NON-NLS-1$
		}
		else
		{
			application.reportInfo("Solution tried to close the developer"); //$NON-NLS-1$
		}
	}
	...
	public String js_executeProgram(Object[] vargs) throws ApplicationException
	{
		...
		try
		{
			...
			for (Object element : vargs)
			{
				...
				if ("#".equals(arg) && currentArgs == cmdArgs) //$NON-NLS-1$
				{
					currentArgs = envArgs;
				}
				else if (("#".equals(arg) || "|".equals(arg)) && currentArgs == envArgs) //$NON-NLS-1$ //$NON-NLS-2$
				{
...
//			application.reportError(application.getI18NMessage("servoy.error.executing.program") + vargs[0], e); //$NON-NLS-1$
		}
	 ...
	 *
	 * @deprecated use {@link com.servoy.j2db.FormController.JSForm#js_getContainerName()} in order to check this.
	 * @sample
	...
	public void js_showFormInDialog(Object[] args) throws ServoyException
	{
		if (args != null && args.length >= 1)
		{
			...
			if (f != null)
			{
				...
				Rectangle rect;
				if (x == IApplication.FULL_SCREEN || y == IApplication.FULL_SCREEN || w == IApplication.FULL_SCREEN || h == IApplication.FULL_SCREEN)
				{
					rect = FormManager.FULL_SCREEN;
				}
				else
				{
	...
	public void js_showFormInWindow(Object[] args) throws ServoyException
	{
		...
		if (args != null && args.length >= 1)
		{
			...
			if (formName != null)
			{
				...
				if (x == IApplication.FULL_SCREEN || y == IApplication.FULL_SCREEN || w == IApplication.FULL_SCREEN || h == IApplication.FULL_SCREEN)
				{
					bounds = FormManager.FULL_SCREEN;
				}
				else
				{
	 ...
	 *
	 * @deprecated use closeForm(...) instead.
	 * @sample
	...
	public boolean js_closeForm(Object[] vargs)
	{
		...
		if (vargs != null && vargs.length >= 1)
		{
			if (vargs[0] instanceof Boolean)
			{
				...
				if (all)
				{
					return fm.closeFormInDialogOrWindow("", true); //$NON-NLS-1$
				}
			}
		...
		boolean closed = fm.closeFormInDialogOrWindow(dialogName, all);
		if (closed)
		{
	...
	{
		return "JSApplication[Standard functions]"; //$NON-NLS-1$
	}
