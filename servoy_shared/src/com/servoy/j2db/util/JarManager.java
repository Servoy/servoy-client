...
{
	public static void addCommonPackageToDefinitions(Extension[] exts, Map _loadedBeanDefs)
	{
		String lastJarFileName = null;
		List workingClassNames = new ArrayList();
		...
		for (int i = 0; i < exts.length; i++)
		{
			if (lastJarFileName != null && !exts[i].jarFileName.equals(lastJarFileName))
			{
				...
				workingClassNames = new ArrayList();
			}
	...
	private static void addCommonPackageToDefinitions(String fileName, List workingClassNames, Map _loadedBeanDefs)
	{
		...
		do
		{
			...
			Iterator it = workingClassNames.iterator();
			while (it.hasNext())
			{
...
//					Debug.trace("commonPart "+commonPart);
	...
	public static Class[] getAssignableClasses(ExtendableURLClassLoader loader, Class type, Map searchURLs) throws IOException
	{
...
//		public boolean enabled = true;
	...
	public static Extension[] getExtensions(ExtendableURLClassLoader loader, Class searchType, Map<URL, Pair<String, Long>> searchURLs) throws IOException
	{
	...
	public static Map loadLibs(File dir)
	{
	...
	public static List readDir(File dir, Map baseRetval, Map subDirRetval, Map packageJarMapping, boolean isSubDir)
	{
		List foundBeanClassNames = new ArrayList();
			...
			for (int i = 0; i < files.length; ++i)
			{
				...
				if (fileName.toLowerCase().endsWith(".zip") || fileName.toLowerCase().endsWith(".jar")) //$NON-NLS-1$ //$NON-NLS-2$
				{
					try
					{
						...
						if (isSubDir)
						{
							subDirRetval.put(jarFile.toURI().toURL(), new Pair(fileName, new Long(jarFile.lastModified())));
						}
						else
						{
							baseRetval.put(jarFile.toURI().toURL(), new Pair(fileName, new Long(jarFile.lastModified())));
							...
							if (packageJarMapping != null)
							{
								...
								if (mf != null)
								{
									List beanClassNames = getBeanClassNames(mf);
										...
										if (classpath != null)
										{
											...
											while (st.hasMoreTokens())
											{
												...
												if (classPathFile.exists())
												{
													...
													subDirRetval.put(classPathFile.toURI().toURL(), new Pair(classPathJar, new Long(
	...
	private static List getBeanClassNames(Manifest mf)
	{
		...
		int i = 0;
		Iterator it2 = beans.keySet().iterator();
		while (it2.hasNext())
		{
			i++;
	...
	private static void checkIfBeanAttribute(Attributes attr, String key, Map beans)
	{
		if (attr == null) return;
