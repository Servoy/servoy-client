...
public class J2DBClient extends ClientState implements ISmartClientApplication, IGlobalEditListener, IInfoListener, IReconnectListener, IMessagesCallback
{
	...
	protected void createMainPanel()
	{
		...
		frame.setName("Application_frame"); //$NON-NLS-1$
...
//				getFlattenedSolution().setSolution(null);
		...
		try
		{
			...
			invokeLater(new Runnable()// make some stuff later null so its not created again
			{
				public void run()
				{
					...
					// only show the open solution dialog on this point if there is 
					// not already again a solution opened.
					if (getSolution() == null)
					{
						handleClientUserUidChanged(null, null); // TODO: pass in real uid?
					}
				}
			});
	...
	{
		Window window = getWindow(IFormManagerInternal.USER_WINDOW_PREFIX + FormManager.DEFAULT_DIALOG_NAME);
	...
	/**
	 * Removes all user dialogs (created through JS) from cache.
	 */
	public void clearUserWindows()
	{
		// delete all user dialogs - maybe you do not want to use the stored bounds any longer for ex.
		Iterator<Map.Entry<String, Window>> it = dialogs.entrySet().iterator();
		while (it.hasNext())
		{
			Map.Entry<String, Window> item = it.next();
			String key = item.getKey();
			if (key.startsWith(IFormManagerInternal.USER_WINDOW_PREFIX))
			{
				Window element = item.getValue();
	...
	protected void bindUserClient()
	{
		try
		{
			...
			Debug.trace("RMI export succeeded on port: " + port); //$NON-NLS-1$
	...
	public synchronized int exportObject(Remote object) throws RemoteException
	{
		...
		if (counter != -1)
		{
			Debug.error("Couldnt export object with port from server, trying to do on anonym port"); //$NON-NLS-1$
			rmiExportPort = 0;
			UnicastRemoteObject.exportObject(object, rmiExportPort, rmiFactoryFactory.getClientSocketFactory(), rmiFactoryFactory.getServerSocketFactory());
		}
	...
	public Rectangle getWindowBounds(String windowName)
	{
		...
		else
		{
			...
			w = getWindow(IFormManagerInternal.USER_WINDOW_PREFIX + windowName.replace(' ', '_'));
		}
	...
	public String showI18NDialog(String preselect_key, String preselect_language)
	{
		...
		if (sfd == null || sfd.getOwner() != window)
		{
			...
			registerWindow("I18NDialog_main", sfd); //$NON-NLS-1$
		}
		else
		{
			sfd.setEndUser(true);
	...
	public Date showCalendar(String pattern, Date date)
	{
		...
		if (chooser == null || SwingUtilities.getWindowAncestor(chooser) != windowParent)
		{
			if (chooser != null)
			{
				chooser.dispose();
				chooser = null;
				registerWindow("JDateChooser", chooser); //$NON-NLS-1$
			}
	...
	public void setWindowSize(int width, int height, String windowName)
	{
		Window w = getUserWindow(windowName);
	...
	public void setWindowLocation(int x, int y, String windowName)
	{
		Window w = getUserWindow(windowName);
		...
		{
			Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
			Dimension frameSize = getMainApplicationFrame().getSize();
			x = (screenSize.width / 2) - (frameSize.width / 2);
			y = (screenSize.height / 2) - (frameSize.height / 2);
		}
	...
	public String showColorChooser(String originalColor)
	{
		...
		if (ccd == null || ccd.getOwner() != getMainApplicationFrame())
		{
			...
			registerWindow("CustomColorChooserDialog", ccd); //$NON-NLS-1$
		}
	...
	public void loadSkin()
	{
		...
		{
			Debug.error(ex);
			reportError(getI18NMessage("servoy.formManager.error.LoadingSkin"), ex); //$NON-NLS-1$
		}
		loadedSkin = true;
	}
