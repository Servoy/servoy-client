...
public class NGClientWebsocketSession extends BaseWebsocketSession implements INGClientWebsocketSession
{
	...
	protected IClientService createClientService(String name)
	{
		...
		ServoyClientService clientService = new ServoyClientService(name, spec, this);
		...
		for (String propName : spec.getAllPropertiesNames())
		{
			...
			try
			{
				...
				if (pd.hasDefault())
				{
					...
					formElementEquivalentValue = NGConversions.INSTANCE.convertDesignToFormElementValue(pd.getDefaultValue(), pd, client.getFlattenedSolution(),
						null, propertyPath);
				...
				else if (pd.getType() instanceof IDesignDefaultToFormElement< ? , ? , ? >)
				{
					...
					formElementEquivalentValue = ((IDesignDefaultToFormElement< ? , ? , ? >)pd.getType()).toDefaultFormElementValue(pd,
						client.getFlattenedSolution(), null, propertyPath);
					propertyPath.backOneLevel();
				}
				...
				if (formElementEquivalentValue != null)
				{
					...
					clientService.setProperty(propName,
						NGConversions.INSTANCE.convertFormElementToSabloComponentValue(formElementEquivalentValue, pd, null, null, null));
		...
		clientService.propertiesInitialized();
