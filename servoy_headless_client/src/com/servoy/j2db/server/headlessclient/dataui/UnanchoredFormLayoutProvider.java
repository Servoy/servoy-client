 ...
 * @author gerzse
 */
public class UnanchoredFormLayoutProvider extends AbstractFormLayoutProvider
{
	public UnanchoredFormLayoutProvider(IServiceProvider sp, Solution solution, Form f, String formInstanceName)
	{
	...
	{
		if (orientation.equals(OrientationApplier.RTL)) partStyle.setProperty("right", defaultNavigatorShift + "px"); //$NON-NLS-1$ //$NON-NLS-2$
		else partStyle.setProperty("left", defaultNavigatorShift + "px"); //$NON-NLS-1$ //$NON-NLS-2$
		partStyle.setProperty("width", "100%"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		int height = part.getHeight() - top;
		if (part.getPartType() == Part.BODY)
		{
			height += spaceUsedOnlyInPrintAbove + spaceUsedOnlyInPrintBelow;
		}
		partStyle.setProperty("height", height + "px"); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public TextualStyle getLayoutForForm(int customNavigatorWidth, boolean isNavigator, boolean isInTabPanel)
	{
		...
		if (customNavigatorWidth > 0)
		{
			...
			else formStyle.setProperty("width", "100%"); //$NON-NLS-1$ //$NON-NLS-2$
			formStyle.setProperty("top", "0px"); //$NON-NLS-1$ //$NON-NLS-2$
			formStyle.setProperty("height", "100%"); //$NON-NLS-1$ //$NON-NLS-2$
		}
