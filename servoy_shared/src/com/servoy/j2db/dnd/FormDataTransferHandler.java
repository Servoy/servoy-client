...
public class FormDataTransferHandler extends TransferHandler implements DropTargetListener
{
	...
	public int getSourceActions(JComponent c)
	{
		...
		JSEvent event = createScriptEvent(EventType.onDrag, (ISupportDragNDrop)c, inputEvent);
		int dragReturn = ddComp.onDrag(event);
	...
	public boolean canImport(JComponent comp, DataFlavor[] transferFlavors)
	{
		...
		if (isDataFlavorSupported(transferFlavors) && cmp instanceof ISupportDragNDrop)
		{
			...
			JSEvent event = createScriptEvent(EventType.onDragOver, ddComp, dropTargetDragEvent);
...
//	}
	...
	public boolean importData(JComponent comp, Transferable t)
	{
		...
		if (cmp instanceof ISupportDragNDrop)
		{
			...
			try
			{
				JSEvent event = createScriptEvent(EventType.onDrop, ddComp, dropTargetDropEvent);
//				ddComp = testTarget(ddComp, event);
				event.setData(eventData);
				eventData = null;
	...
	private JSEvent createScriptEvent(EventType type, ISupportDragNDrop ddComponent, Object event)
	{
	...
	static class TransferAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (src instanceof JComponent)
			{
				...
				TransferHandler th = (src instanceof ISupportDragNDropTextTransfer) ? ((ISupportDragNDropTextTransfer)src).getTextTransferHandler() : c.getTransferHandler();
