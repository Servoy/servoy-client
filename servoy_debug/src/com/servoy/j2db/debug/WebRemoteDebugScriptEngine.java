...
import com.servoy.j2db.server.headlessclient.FunctionEvent;
import com.servoy.j2db.server.headlessclient.IFunctionExecutor;
import com.servoy.j2db.server.headlessclient.ScriptExecutor;

/**
 * Special ScriptEngine for debugging that implements {@link IFunctionExecutor} executing scripts in a separate Thread.
 * 
 * @author jcompagner
 * 
 * @since 6.1
 */
public class WebRemoteDebugScriptEngine extends RemoteDebugScriptEngine implements IFunctionExecutor
{
	 ...
	 * @param app
	 */
	public WebRemoteDebugScriptEngine(IApplication app)
	{
		super(app);
		...
		throws Exception
	{
		final FunctionEvent event = new FunctionEvent(this, f, scope, thisObject, args, focusEvent, throwException)
		{
			 ...
			 * (non-Javadoc)
			 * 
			 * @see com.servoy.j2db.server.headlessclient.ExecuteEvent#execute()
			 */
			@Override
			public void execute()
			{
				try
				{
					((DebugWebClient)application).addEventDispatchThread();
					super.execute();
				}
				finally
				{
					((DebugWebClient)application).removeEventDispatchThread();
				}
			}
		};
