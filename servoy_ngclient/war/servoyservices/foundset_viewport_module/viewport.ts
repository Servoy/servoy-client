	...
	function updateViewportGranularly(viewPort, internalState, rowUpdates, rowUpdateConversions, componentScope,
		...
		for (i = 0; i < rowUpdates.length; i++) {
			...
			if (rowUpdate.type == CHANGE) {
                ...
                // apply conversions
				if (rowUpdateConversions && rowUpdateConversions[i]) rowUpdate = $sabloConverters.convertFromServerToClient(rowUpdate, rowUpdateConversions[i], undefined, componentScope, propertyContext);
