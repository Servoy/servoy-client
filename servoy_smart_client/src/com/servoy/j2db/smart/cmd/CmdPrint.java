...
public class CmdPrint extends AbstractCmd
{
	...
	{ 
		super(app,"CmdPrint",app.getI18NMessage("servoy.menuitem.print"),"servoy.menuitem.print",app.getI18NMessage("servoy.menuitem.print.mnemonic").charAt(0),app.loadImage("print.gif")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
	...
	{
/*		Editor ce = application.getEditor();
		...
		if (pjob != null) {
			...
			for (int y=0; y <= drawingArea.height; y+=printableHeight-OVERLAP) {
				for (int x=0; x <= drawingArea.width; x+=printableWidth-OVERLAP) {
					...
					application.setStatusText("Printing page " + pageNum);
		...
		application.setStatusText("Printing finished");
*/
		return null;
	}
}
