...
public class MethodTemplate
{
	...
	public String getMethodDeclaration(CharSequence name, CharSequence methodCode)
	{
		...
		if (args != null && args.length > 0)
		{
			...
			for (MethodArgument element : args)
			{
				...
				if (element.getType() != null && element.getType() != MethodArgument.ArgumentType.Other)
				{
		...
		if (signature != null && signature.getType() != null && signature.getType() != MethodArgument.ArgumentType.Other)
		{
	...
	public Element toXML()
	{
		...
		if (signature.getType() != null) root.addAttribute(ATTR_TYPE, signature.getType().name());
