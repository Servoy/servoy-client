...
abstract class RTFParser extends AbstractFilter
{
    ...
    throws IOException
  {
    ...
    switch (state)
    {
	  ...
	  if (level == 0)
	    throw new IOException("Too many close-groups in RTF text"); //$NON-NLS-1$
          endgroup();
	  level --;
	} else if(ch == '\\') {
	  if (currentCharacters.length() > 0) {
	...
	if (!Character.isLetter(ch)) {
	  ...
	  if (!handleKeyword(new String(newstring))) {
	    warning("Unknown keyword: " + new String(newstring) + " (" + (byte)ch + ")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	  }
	  state = S_text;
	  pendingKeyword = null;
	...
	if (Character.isLetter(ch)) {
	  ...
	  if (Character.isDigit(ch) || (ch == '-')) {
	    ...
	    if (!ok)
	      warning("Unknown keyword: " + pendingKeyword); //$NON-NLS-1$
	    pendingKeyword = null;
	    state = S_text;
	...
	if (Character.isDigit(ch)) {
	  ...
	  if (pendingKeyword.equals("bin")) {  /* magic layer-breaking kwd */ //$NON-NLS-1$
	  ...
	  if (!ok)
	    warning("Unknown keyword: " + pendingKeyword + //$NON-NLS-1$
		    " (param " + currentCharacters + ")"); //$NON-NLS-1$ //$NON-NLS-2$
	  pendingKeyword = null;
	  currentCharacters = new StringBuffer();
	  state = S_text;
    ...
    throws IOException
  {
    ...
    if (state != S_text || level > 0) {
      warning("Truncated RTF file."); //$NON-NLS-1$
