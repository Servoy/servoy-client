...
public class MultiSelectionPanel extends JPanel implements ActionListener
{
	...
	private final boolean hasUpDownButtons;
	...
	private final DefaultListModel targetModel = new DefaultListModel();

	private IndexedObject[] entryList;

	public MultiSelectionPanel(Object[] entries, boolean[] isSelected, int[] targetListIndices, String message, String label1, String label2,
		boolean hasUpDownButtons)
	{
		super();
		...
		sourceList.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(MouseEvent e)
			{
				...
				{
					if (MultiSelectionPanel.this.hasUpDownButtons) add(); // order matters in target
	...
	public void setData(Object[] entries, boolean[] isSelected, int[] targetListIndices)
	{
		...
		if (!hasUpDownButtons) Arrays.sort(targetListIndices);
		entries = entries.clone();
		for (int i = 0; i < targetListIndices.length; i++)
		{
			int index = targetListIndices[i];
			targetModel.addElement(entryList[index]);
			if (isSelected != null && isSelected[index]) targetList.addSelectionInterval(i, i);
			entries[index] = null;
		}
		...
		for (int i = 0, j = 0; i < entries.length; i++)
		{
			if (entries[i] != null)
			{
				...
				if (isSelected != null && isSelected[i]) sourceList.addSelectionInterval(j, j);
				j++;
			}
		}
	}
	...
	private void add()
	{
		...
		if (selected.length > 0)
		{
			...
			{
				Object object = sourceModel.remove(element - j++);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			if (hasUpDownButtons) add(); // order matters in target
			else move(sourceList, targetList); // keep standard order
		}
