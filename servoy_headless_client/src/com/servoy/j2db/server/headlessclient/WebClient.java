...
public class WebClient extends SessionClient implements IWebClientApplication
{
	...
	protected WebClient(HttpServletRequest req, Credentials credentials, String method, Object[] methodArgs, String solution) throws Exception
	{
		...
		ci.setTimeZone(TimeZone.getDefault()); // server time zone for webclients in the client info for better use of the useClientTimeZone property
		//ci.setTimeZone(getTimeZone());
		getTimeZone(); // cache the time-zone in the beginning, because otherwise scheduler tasks may fail (when performing find for example) - if executed before the first
